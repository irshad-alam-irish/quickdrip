...
def custom_openapi():
    ...
    # Apply security to all endpoints that use get_current_user dependency
    for path_data in openapi_schema["paths"].values():
        for operation in path_data.values():
            if isinstance(operation, dict):
                # Check if this operation has security (endpoints with get_current_user)
                # We'll apply security to all non-auth endpoints
